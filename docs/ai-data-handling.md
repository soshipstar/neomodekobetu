...
- `ai_generation_logs` に user_id / generation_type / model / **temperature / max_tokens / system_prompt(=版) / parameters(response_format・参照情報 referenced・finish_reason)** / トークン数 / 入力(マスク済・先頭5000字) / 出力(マスク済) / 所要時間 / 作成日時を記録（追記専用: `UPDATED_AT = null`）。
- `vector_embeddings.metadata` に **embedding_model / embedded_at** を記録し、埋め込みの再現性・鮮度を追跡可能にした（観点9 データ品質にも寄与）。
- 残課題: 生成物（個別支援計画等）の作成→修正→承認の版チェーンの全書類への拡張、第三者監査の手順整備。
