...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_monitoring_id'])) {
    ...
    try {
        ...
        if ($studentId && $planId) {
            header("Location: /minimum/staff/kobetsu_monitoring.php?student_id=$studentId&plan_id=$planId");
        } else {
            header("Location: /minimum/staff/kobetsu_monitoring.php");
        }
        exit;
    } catch (Exception $e) {
...
<a href="/minimum/staff/renrakucho_activities.php" class="quick-link">← 活動管理に戻る</a>
            ...
            <?php if ($selectedPlanId && $planData): ?>
                    ...
                    <div class="monitoring-list">
                        ...
                        <?php foreach ($existingMonitorings as $monitoring): ?>
                            <div style="display: inline-flex; align-items: center; gap: 5px;">
                                <a href="/minimum/staff/kobetsu_monitoring.php?student_id=<?= $selectedStudentId ?>&plan_id=<?= $selectedPlanId ?>&monitoring_id=<?= $monitoring['id'] ?>"
                                ...
                                <form method="POST" style="display: inline;" onsubmit="return confirm('このモニタリング表を削除してもよろしいですか？');">
                                    ...
                                    <button type="submit" style="background: var(--md-red); color: white; border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: var(--text-caption-1);"><span class="material-symbols-outlined" style="font-size: 18px; vertical-align: middle;">delete</span></button>
                                </form>
                            </div>
                        <?php endforeach; ?>
                        <a href="/minimum/staff/kobetsu_monitoring.php?student_id=<?= $selectedStudentId ?>&plan_id=<?= $selectedPlanId ?>" class="monitoring-item">+ 新規作成</a>
                    </div>
                <?php endif; ?>
                ...
                <!-- モニタリング入力フォーム -->
                <form method="POST" action="/minimum/staff/kobetsu_monitoring_save.php" id="monitoringForm">
                    <input type="hidden" name="plan_id" value="<?= $selectedPlanId ?>">
                    <input type="hidden" name="student_id" value="<?= $selectedStudentId ?>">
                    ...
                    <div class="button-group">
                        <button type="submit" name="save_draft" class="btn btn-secondary"><span class="material-symbols-outlined" style="font-size: 18px; vertical-align: middle;">edit_note</span> 下書き保存（保護者非公開）</button>
                        <button type="submit" class="btn btn-success"><span class="material-symbols-outlined" style="font-size: 18px; vertical-align: middle;">check_circle</span> 作成・提出（保護者公開）</button>
                    </div>
                </form>
            <?php else: ?>
...
// ページ変数
        ...
        async function generateAllEvaluations() {
            ...
            try {
                const response = await fetch('/minimum/staff/monitoring_generate_api.php', {
                    method: 'POST',
                    headers: {
