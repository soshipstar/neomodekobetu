...
if (!$studentId || !$studentName || !$createdDate) {
    ...
    header("Location: /minimum/staff/kobetsu_plan.php?student_id=$studentId" . ($planId ? "&plan_id=$planId" : ''));
    exit;
}
...
if (!$planId && $student && $student['support_start_date']) {
    ...
    if ($existingPlan) {
        ...
        header("Location: /minimum/staff/kobetsu_plan.php?student_id=$studentId&plan_id={$existingPlan['id']}");
        exit;
    }
}
...
header("Location: /minimum/staff/kobetsu_plan.php?student_id=$studentId&plan_id=$planId");
exit;
