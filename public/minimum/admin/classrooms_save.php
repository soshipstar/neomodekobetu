...
try {
    switch ($action) {
        case 'add':
            ...
            $phone = $_POST['phone'] ?? '';
            $serviceType = $_POST['service_type'] ?? 'normal';
            $logoPath = '';
            ...
            // データベース登録
            $stmt = $pdo->prepare("
                INSERT INTO classrooms (classroom_name, address, phone, logo_path, service_type)
                VALUES (?, ?, ?, ?, ?)
            ");
            $stmt->execute([$classroomName, $address, $phone, $logoPath, $serviceType]);
        ...
        case 'edit':
            ...
            $stmt = $pdo->prepare("
                UPDATE classrooms
                SET classroom_name = ?, address = ?, phone = ?, logo_path = ?, service_type = ?
                WHERE id = ?
            ");
            $stmt->execute([$classroomName, $address, $phone, $logoPath, $serviceType, $classroomId]);
