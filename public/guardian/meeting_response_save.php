...
try {
    ...
    if ($action === 'select' && $selectedDate) {
        ...
        $stmt->execute([$request['student_id'], $guardianId]);
        $room = $stmt->fetch();
        ...
        if ($room) {
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES (?, 'guardian', ?, ?, 'meeting_confirmed', ?, NOW())
            ");
            $stmt->execute([$room['id'], $guardianId, $messageText, $requestId]);
        ...
        $stmt = $pdo->prepare("
            UPDATE meeting_requests SET
                guardian_counter_date1 = ?,
                guardian_counter_date2 = ?,
                guardian_counter_date3 = ?,
        ...
        if ($room) {
            ...
            $messageText .= "申し訳ございませんが、ご提案いただいた日程での都合がつきませんでした。\n";
            ...
            $stmt = $pdo->prepare("
                INSERT INTO chat_messages (room_id, sender_type, sender_id, message, message_type, meeting_request_id, created_at)
                VALUES (?, 'guardian', ?, ?, 'meeting_counter', ?, NOW())
            ");
            $stmt->execute([$room['id'], $guardianId, $messageText, $requestId]);
        ...
        $pdo->commit();
        $_SESSION['success'] = '別日程を提案しました。スタッフからの回答をお待ちください。';
