...
class AssessmentController extends Controller
{
    ...
    public function generate(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // 6 か月分の連絡帳記録を入力に 5 領域 + 短期目標 + 長期目標を生成する
            // 長文タスクなので通常版で品質確保。他箇所と日付を 2026-03-17 に統一。
            $aiModel = 'gpt-5.4-2026-03-05';
            $totalInputTokens = 0;
            $totalOutputTokens = 0;

            // 連絡帳を全件、月別×領域別に蒸留(二段集約)してから課題生成へ渡す。
            // 月別(時系列)構造にするのは「直近の記録=現在の様子」を基準にしつつ、
            // 期間はじめとの差分を成長の記録として明記させるため(取りこぼし防止も維持)。
            $distiller = app(\App\Services\RecordDistillationService::class);
            $distilled = $distiller->distillByDomain($student, $records, $masker, groupByMonth: true);
            ...
            // === 1. 五領域の課題を生成（レガシー準拠 + 時系列分析） ===
            $domainsPrompt = "あなたは発達支援・特別支援教育の専門スタッフです。以下の生徒の直近6か月の連絡帳記録(全件を月別×領域別に要約済み)を時系列で分析し、今後6か月間の具体的な支援課題を各領域ごとにまとめてください。\n\n"
                . "【生徒情報】\n名前: {$student->student_name}\n\n"
                . "【直近6か月の連絡帳記録(全件の要約/各領域内は古い月→新しい月の順)】\n{$recordsSummary}\n\n"
                . (trim($guardianSummary) !== '' ? "【保護者アセスメント(家庭からの視点)】{$guardianSummary}\n" : '')
                . "【時系列分析の指針】\n"
                . "- 直近1〜2か月の記録を「現在の様子」の基準とし、それ以前の記録は経過・変化を把握するための基線(ベースライン)として扱ってください。\n"
                . "- 期間はじめの様子と現在の様子を比較し、成長・改善が読み取れる場合は、その変化を月の根拠とともに必ず記述してください(例: ○月時点では〜だったが、△月には〜できるようになった)。\n"
                . "- 過去に見られた課題が直近の記録に登場しない場合、「解決した」と断定せず、「直近の記録では確認されていない」など事実の範囲で記述してください。\n"
                . "- 記録にある事実のみを用い、推測や創作はしないでください。\n\n"
                . "【分析と課題作成の指針】\n"
                . "以下の5つの領域について、「期間はじめの様子 → 現在の様子 → 変化(成長・継続課題) → 今後6か月の課題」の流れを意識し、記録から読み取れる具体的な事実を基に300〜400文字程度で詳細に記述してください。\n\n"
                ...
                . "■ 人間関係・社会性\n- 他者への関心、対人距離、集団参加の様子\n- 感情表現、感情調整、共感性の発達\n- 友だち関係、協調性、ルール理解の状況\n\n"
                . "**重要**: 以下のJSON形式のみを出力してください。各領域300〜400文字程度で、観察事実に基づく具体的な内容（期間中の変化・成長を含む）を記述してください。\n"
                . "{\n"
                . "  \"health_life\": \"健康・生活の課題（300〜400文字程度。期間中の変化・成長と今後の課題を含む）\",\n"
                . "  \"motor_sensory\": \"運動・感覚の課題（300〜400文字程度。期間中の変化・成長と今後の課題を含む）\",\n"
                . "  \"cognitive_behavior\": \"認知・行動の課題（300〜400文字程度。期間中の変化・成長と今後の課題を含む）\",\n"
                . "  \"language_communication\": \"言語・コミュニケーションの課題（300〜400文字程度。期間中の変化・成長と今後の課題を含む）\",\n"
                . "  \"social_relations\": \"人間関係・社会性の課題（300〜400文字程度。期間中の変化・成長と今後の課題を含む）\"\n"
                . "}";
            ...
            $domainsResponse = $client->chat()->create([
                ...
                'temperature'           => 0.6,
                // 5領域×300〜400字(変化・成長の記述を含む)がJSONで途切れないよう余裕を持たせる
