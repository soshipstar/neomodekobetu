        ...
        foreach ($planDetails as $detail) {
            if (empty($detail->goal)) {
                $generatedEvaluations[$detail->id] = [
                    'achievement_level' => '',
                    'comment' => '支援目標が設定されていないため、評価を生成できません。',
                ];
                continue;
            }
            ...
            if (empty($relatedRecords)) {
                $generatedEvaluations[$detail->id] = [
                    'achievement_level' => '',
                    'comment' => '過去6ヶ月間にこの分野に関連する記録がありませんでした。',
                ];
                continue;
            }
            ...
            try {
                $response = OpenAI::chat()->create([
                    ...
                    'messages' => [
                        [
                            'role'    => 'system',
                            'content' => 'あなたは児童発達支援施設の児童発達支援管理責任者です。モニタリング評価を専門的かつ分かりやすく行います。JSON形式のみで回答してください。',
                        ],
                        [
                            'role'    => 'user',
                            'content' => "以下の支援目標に対して評価してください。\n\n"
                                . "【児童名】{$student->student_name}\n"
                                . "【分野】{$detail->domain}\n"
                                . "【支援目標】{$detail->goal}\n"
                                . "【支援内容】{$detail->support_content}\n\n"
                                . "【過去6ヶ月の記録】\n{$recordsText}\n\n"
                                . "出力: {\"achievement_level\": \"達成/進行中/未着手/継続中/見直し必要\", \"comment\": \"150〜200字の評価\"}",
                        ],
                    ],
                    'temperature'           => 0.5,
                ...
                $generatedEvaluations[$detail->id] = $result ?? [
                    'achievement_level' => '',
                    'comment' => $content,
                ];
            } catch (\Exception $e) {
