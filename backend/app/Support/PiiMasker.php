...
class PiiMasker
{
    ...
    /**
     * 氏名以外の構造化PII(日付・電話・郵便番号・連続数字・敬称付き人物名)を決定的に除去する。
     *
     * 氏名マスク(map置換)では拾えない自由記述中の要配慮情報を、外部AIへ送る前に落とすための
     * 補助スクラバ。S5の例文注入など、第三者の実名・生年月日等が混入しうる経路で使う。
     * 完全ではない(敬称無しの第三者名など残余あり)が、生年月日・連絡先等の典型PIIを塞ぐ。
     */
    public static function scrubStructuredPii(string $text): string
    {
        if ($text === '') {
            return $text;
        }
        // 日付: 2018年4月3日 / 2018/4/3 / 2018-04-03
        $text = (string) preg_replace('/\d{4}\s*[年\/.\-]\s*\d{1,2}\s*[月\/.\-]\s*\d{1,2}\s*日?/u', '【日付】', $text);
        // 月日: 4月3日
        $text = (string) preg_replace('/\d{1,2}\s*月\s*\d{1,2}\s*日/u', '【日付】', $text);
        // 電話: 03-1234-5678 / 090-1234-5678 / 0123-45-6789
        $text = (string) preg_replace('/0\d{1,4}[-(]?\d{1,4}[-)]?\d{3,4}/u', '【電話】', $text);
        // 郵便番号: 123-4567
        $text = (string) preg_replace('/\d{3}-\d{4}/u', '【番号】', $text);
        // 連続4桁以上の数字(各種ID・番号)
        $text = (string) preg_replace('/\d{4,}/u', '【番号】', $text);
        // 敬称付きの人物名(他児・きょうだい・医師等): 〇〇さん/くん/ちゃん/君/様/先生/医師
        // 文字クラスは漢字・かなの「文字」のみに限定する(。【】等のCJK記号は script-extension で
        // \p{Hiragana} 等に含まれ、隣接プレースホルダを巻き込むため明示レンジで除外)。
        $text = (string) preg_replace('/[\x{4E00}-\x{9FFF}\x{3041}-\x{3096}\x{30A1}-\x{30FA}\x{30FC}々]{1,5}(さん|くん|ちゃん|君|様|先生|医師)/u', '【人物】$1', $text);
