...
class KakehashiService
{
    ...
    /**
     * 次のアセスメント期間を自動生成すべきか確認
     * 次のアセスメントの提出期限の1ヶ月前になったら生成する
     *
    ...
    /**
     * support_plan_start_type='next' の生徒で、初回アセスメント生成のタイミングかチェック
    ...
    /**
     * 定期的に次のアセスメント期間を自動生成（スケジュール実行）
     * 最新の期限の1ヶ月前になったら次のアセスメント期間を生成する
     *
    ...
    public function autoGenerateNextKakehashiPeriods(): array
    {
        ...
        // support_plan_start_type='next' でまだアセスメントがない生徒も処理
        // 次回の期間（2回目相当）の提出期限が近づいたら、1回目のアセスメントから生成
    ...
    /**
     * support_plan_start_type を無視してアセスメント期間を強制生成
    ...
    public function generateNextKakehashiPeriod(int $studentId, string $studentName): ?array
    {
        ...
        if (!$latestPeriod) {
            throw new \Exception("既存のアセスメント期間が見つかりません");
        }
    ...
    /**
     * アセスメント期間に対応するモニタリングシートを自動作成
     * 最新の個別支援計画の内容をコピーして、評価欄のみ編集可能なモニタリングを作成
     *
     * @param int $studentId 生徒ID
     * @param string $monitoringDate モニタリング実施日（アセスメント提出期限）
     */
