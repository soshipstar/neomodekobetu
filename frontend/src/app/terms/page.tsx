...
export default function TermsPage() {
  return (
    <div className="mx-auto min-h-screen max-w-3xl bg-white px-5 py-10 text-[var(--neutral-foreground-1)]">
      ...
      <Section n="第6条" title="生成AIの利用について">
        <ul className="list-disc space-y-1 pl-5">
          <li>
            本サービスは、個別支援計画・モニタリング・連絡帳・おたより等の<strong>書類作成の下書き支援</strong>を目的として、
            生成AI（OpenAI のAPI）を利用します。AIは作成補助であり、<strong>医療上の診断・治療判断には使用しません</strong>。
          </li>
          <li>
            AIが作成した内容は<strong>下書き（素案）</strong>であり、必ず事業所の専門職員が確認・修正したうえで正式な書類とします。
            最終的な内容の責任は事業所が負います。
          </li>
          <li>
            AIへ送信する際は、<strong>児童・保護者の氏名等を仮名化（マスキング）</strong>したうえで送信し、実名等の個人情報の外部送信を最小化します。
          </li>
          <li>
            AIの提供事業者（OpenAI）は、本サービスにとっての業務委託先（データ処理者）であり、送信データはAIモデルの学習には使用されません。
            詳細は別途定めるプライバシーポリシーおよびデータ取扱い方針に従います。
          </li>
        </ul>
      </Section>

      <Section n="第7条" title="違反時の措置">
        <ul className="list-disc space-y-1 pl-5">
          <li>
