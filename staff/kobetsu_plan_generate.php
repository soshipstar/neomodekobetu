...
try {
    ...
    // デフォルト日付を計算
    $submissionDeadline = new DateTime($period['submission_deadline']);
    $createdDate = clone $submissionDeadline;
    $createdDate->modify('+1 day'); // 提出期限の翌日

    $longTermGoalDate = clone $createdDate;
    $longTermGoalDate->modify('+1 year'); // 1年後

    $shortTermGoalDate = clone $createdDate;
    $shortTermGoalDate->modify('+6 months'); // 半年後

    $achievementDate = clone $createdDate;
    $achievementDate->modify('+6 months'); // 半年後（各領域の達成時期）

    $consentDate = clone $createdDate; // 作成日と同一日
    ...
    $prompt .= '  "long_term_goal_text": "長期目標の内容（施設での支援を通じて到達してほしい具体的な姿を、観察可能な行動として100-150文字程度で記述。施設内での活動・場面を想定した目標にすること。「1年後には」「○ヶ月後には」などの期間を含めた表現は使用しないこと）",'."\n";
    $prompt .= '  "short_term_goal_text": "短期目標の内容（施設での支援を通じて到達してほしい具体的な姿を、観察可能な行動として100-150文字程度で記述。施設内での活動・場面を想定した目標にすること。「半年後には」「○ヶ月後には」などの期間を含めた表現は使用しないこと）",'."\n";
    $prompt .= '  "details": ['."\n";
    $prompt .= '    {'."\n";
...
// リダイレクト先を確認
$redirectUrl = "Location: kobetsu_plan.php?student_id=$studentId";
